...
public class ToolDiscoveryService {
    ...
    public List<AiToolVO> listTools(String cluster) {
        ...
        Set<String> capabilities = capabilityResolver.resolve(instance.getName());

        return catalog.list().stream()
                .filter(definition -> definition.requiredCapabilities().isEmpty()
                        || definition.requiredCapabilities().stream().anyMatch(capabilities::contains))
