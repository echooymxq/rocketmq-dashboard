...
public class ToolExecutionService {
    ...
    private Object executeInternal(String name, Map<String, Object> input, McpAuthentication authentication) {
        try {
            ...
            String cluster = null;
            if (requiresCluster(definition)) {
                Object value = input == null ? null : input.get("cluster");
                if (!(value instanceof String c) || c.isBlank()) {
                    throw ToolError.TOOL_CLUSTER_REQUIRED.exception();
                }
                cluster = c;
                if (authentication != null) {
                    if (!c.equals(authentication.cluster())) {
                        throw ToolError.TOOL_TARGET_MISMATCH.exception();
                    }
                } else {
                    String target = c;
                    instanceResolver.findByName(target)
                            .orElseThrow(() -> ToolError.INSTANCE_NOT_FOUND.exception(target));
                }
            }
    ...
    /**
     * Reports whether the tool's inputSchema declares a "cluster" property.
     * Tools without a cluster field (e.g. rmq.cluster.list) do not require
     * a cluster target.
     */
    private static boolean requiresCluster(ToolDefinition definition) {
        return definition.inputSchema() instanceof Map<?, ?> props
                && props.get("properties") instanceof Map<?, ?> properties
