...
@Import({AuthWebConfig.class, CorsConfig.class, CacheConfig.class, LegacyJackson2Config.class})
class AuthCorsIntegrationTest {
    ...
    void shouldRejectNonAdminMutationBeforeControllerExecution() throws Exception {
        ...
        LoginVO.UserInfo reader = LoginVO.UserInfo.builder()
                .userId(1L).username("reader").admin(false).build();
        when(authService.getAuthenticatedUser(authorization)).thenReturn(Optional.of(reader));
